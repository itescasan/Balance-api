...
namespace Balance_api.Controllers.Contabilidad
{
    public class AsientoController : Controller
    {
        ...
        {
            return V_GetReporte(IdAsiento, NoDoc, IdMoneda, Exportar, Consolidado, Unificado, Modulo);
        }

        private string V_GetReporte(int IdAsiento, string NoDoc, string IdMoneda, bool Exportar, bool Consolidado, bool Unificado, bool Modulo)
        {
            string json = string.Empty;
            ...
            try
            {
                ...
                if(Exportar)
                {
                    ...
                    sqlDataSource.Queries["CNT_RPT_AsientoContable"].Parameters["@P_NoDocumento"].Value = NoDoc;
